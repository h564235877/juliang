...
Sql="select convert(varchar(10),addtime,120) as adddate from CFWztg_AdUser_Order Where UserName='"&UserName&"'"
...
Sql="select count(1) as ordernum,IsNull(sum(ordercent),0) as ordercent,IsNull(sum(ordercent * ordertcrate/100),0) as tccent from CFWztg_AdUser_Order Where UserName='" & UserName & "' and Datediff(d,'"&Rs("AddDate")&"',AddTime)=0 And (OrderState=1 or OrderState=3)"
If AdUserName<>"" Then Sql=Sql&" And AdUserName='"&AdUserName&"'"
...
%>
  <tr> 
    ...
    <td><a href="?Action=orderview&AddTime=<%=Rs("AddDate")%>" class="btn_add"><i class="mdi mdi-eye"></i> 查看</a>
	<a href="?Action=orderlowout&AdUserName=<%=AdUserName%>&AddTime=<%=Rs("AddDate")%>" class="btn_add"><i class="fa fa-sign-out"></i> 导出下级联盟代理数据</a>
	</td>
  </tr>
<%
Rs2.Close
