...
Sql="SELECT "
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan) PlanNum_0,"
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan Where PlanState=1) PlanNum_1,"
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan Where PlanState=2) PlanNum_2,"
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan Where PlanState=3) PlanNum_3,"
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan Where PlanState=4) PlanNum_4,"
Sql=Sql&"(SELECT Count(1) FROM CFWztg_Plan Where PlanState=5) PlanNum_5"
...
Sql="Select * From CFWztg_Plan Where 1=1"
...
<a href="javascript:;" onclick="$(function () { confirm('暂停计划后，此计划所有广告都会暂停投放！确定要暂停吗?', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=planstop&Plan_ID=<%=Rs("ID")%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-stop"></i>暂停</a>

<%ElseIf Rs("PlanState")=5 Then %>
<a href="javascript:;" onclick="$(function () { confirm('投放计划后，此计划所有广告都会开始投放！确定要投放吗?', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=planstart&Plan_ID=<%=Rs("ID")%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-play"></i>投放</a>

<%End If%>

<a href="javascript:;" onclick="$(function () { confirm('如果计划下存在正在运行的广告或计划在审核状态的时候则无法删除成功，需先删除计划下的所有广告后才能删除计划!如计划删除成功，计划内的余额会退回账户内！确定要删除吗?', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=plandel&Plan_ID=<%=Rs("ID")%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-window-close"></i>删除</a>
