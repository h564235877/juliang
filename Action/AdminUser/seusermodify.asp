...
<table class="tb_1">
  ...
  <form name="form4" method="post" action="../../AdminUser.aspx?Action=seusermodifysave&SeUserName=<%=SeUserName%>&RefUrl=<%=Server.URLEncode(Request.ServerVariables("Http_Referer"))%>">
    <tr> 
      <td style="width:20%" class="td_r">客服名：</td>
      <td><%=Rs("UserName")%></td>
    </tr>
    <tr> 
...
<option value="3"<%If Rs("UserState")=3 Then response.write " selected"%>>管理员审核中</option>
	...
	<tr> 
      ...
      <td><input name="UserTcRate" type="text" id="UserTcRate" value="<%=Rs("UserTcRate")%>">
        %(0-100之间，0为不得回扣，100为得和网站主相同收入)</td>
    </tr>
