...
%>
      ...
      <td><select name="AdDuct" id="AdDuct">
          <option value=""<% If CStr(AdDuct)="" Then Response.write " selected"%>>全部</option>
		  <option value="0"<% If CStr(AdDuct)="0" Then Response.write " selected"%>>未扣点</option>
          <option value="1"<% If CStr(AdDuct)="1" Then Response.write " selected"%>>已扣点</option>
        </select></td>
...
<a href="javascript:;" onclick="$(function () { confirm('确定要删除<%=AddDate%>此天的全部有效IP吗??', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=ipdel&AddDate=<%=AddDate%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-window-close"></i> 删除<%=AddDate%>此天的全部有效</a>

<a href="javascript:;" onclick="$(function () { confirm('确定要删除<%=AddDate%>此天以前的全部有效IP??', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=ipbeforedel&AddDate=<%=AddDate%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-window-close"></i> 删除<%=AddDate%>此天以前的全部有效</a>
