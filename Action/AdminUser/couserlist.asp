...
Sql="Select Top 2000 * From CFWztg_User where UserType=4"
...
<a href="?Action=couseradd" class="actionBtn add"><i class="mdi mdi-plus"></i><span>商务新增</span></a>
...
<table class="tb_1">
  <tr class="tr_1"> 
    ...
    <td><a href="<%=PageUrl%>&Px=ID">商务ID</a></td>
    <td><a href="<%=PageUrl%>&Px=UserName">商务名</a></td>
    ...
    <td><a href="<%=PageUrl%>&Px=UserTotal">名下广告主总数</a></td>
...
%>
      ...
      <td>
<a href="?Action=cousermodify&CoUserName=<%=rs("UserName")%>" class="btn_edit"><i class="mdi mdi-pencil"></i>修改</a>
<a href="javascript:;" onclick="$(function () { confirm('如名下还有网站主时客服无法删除，请先转移。删除前请确定此客服没有剩余金额，确定要删除吗?', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdminUser.aspx?Action=couserdel&CoUserName=<%=rs("UserName")%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-window-close"></i>删除</a>
<a href="?Action=couserlowmove&CoUserName=<%=rs("UserName")%>" class="btn_add"><i class="mdi mdi-cursor-move"></i>转移</a>
<a href="?Action=cousergo&CoUserName=<%=rs("UserName")%>" target="_blank" class="btn_add"><i class="mdi mdi-eye"></i>查看</a>
</td>
    </tr>
    <%
Rs.MoveNext
