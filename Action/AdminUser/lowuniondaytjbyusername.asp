...
<%
Sql="Select Sum(a.ClickUv_Counter) As A_ClickUv_Counter,"

Sql=Sql & "Sum(c.Display_Counter) As C_Display_Counter,(Sum(a.ClickUv_Counter)*1000 / case Sum(c.Display_Counter) when 0 then 1 else Sum(c.Display_Counter) end ) C_Display_Counter_Rate,"
Sql=Sql & "Sum(c.Pv_Counter) As C_Pv_Counter,(Sum(a.ClickUv_Counter)*1000 / case Sum(c.Pv_Counter) when 0 then 1 else Sum(c.Pv_Counter) end ) C_Pv_Counter_Rate"

Sql=Sql & " From CFWztg_LowUnion_Counter_Day a"
Sql=Sql & " inner join cfwztg_user b on a.LowUnionUserName=b.UserName inner join (select max(UserName) UserName,max(AddDate) AddDate,sum(Display_Counter) Display_Counter,sum(Pv_Counter) Pv_Counter from CFWztg_AD_Counter_Day where AddDate='"&AddDate&"' group by username) c"
Sql=Sql & " on a.UserName='mytest' and a.LowUnionUserName=c.UserName and a.AddDate=c.AddDate Where  a.AddDate='"&AddDate&"'"
...
Sql="Select max(b.id) UserID,a.LowUnionUserName,Sum(a.ClickUv_Counter) As A_ClickUv_Counter,"
