...
<table>
  <tr>
    <td>名下广告主：<input name="UserName" type="text" id="UserName" value="<%=UserName%>" size="10" /></td>
    <td>开始日期：</td>
...
<table class="tb_2">
<tr>
<td><span class="title">商务业绩统计 从<%=StartDate%>到<%=EndDate%></span></td>
...
Sql=Sql&"group by a.ad_id) c  inner join CFWztg_User d on c.adusername=d.username and d.upusername='"&CoUserName&"'"&chr(10)

If UserName<>"" Then Sql=Sql&" and d.username='"&UserName&"'"&chr(10)
...
<td>￥<%=FormatNumber(Rs("CoUser_Income_Cent"),2,-1,0,0)%>(￥<%=FormatNumber(Rs("AdUser_Spend_Cent"),2,-1,0,0)%>)</td>
<td>￥<%=FormatNumber(Rs("CoUser_Income_CPC"),2,-1,0,0)%>(￥<%=FormatNumber(Rs("AdUser_Spend_CPC"),2,-1,0,0)%>)</td>
<td>￥<%=FormatNumber(Rs("CoUser_Income_CPM"),2,-1,0,0)%>(￥<%=FormatNumber(Rs("AdUser_Spend_CPM"),2,-1,0,0)%>)</td>
<td>￥<%=FormatNumber(Rs("CoUser_Income_CPA"),2,-1,0,0)%>(￥<%=FormatNumber(Rs("AdUser_Spend_CPA"),2,-1,0,0)%>)</td>
<td>￥<%=FormatNumber(Rs("CoUser_Income_CPS"),2,-1,0,0)%>(￥<%=FormatNumber(Rs("AdUser_Spend_CPS"),2,-1,0,0)%>)</td>
<td></td>
...
Sql=Sql&"d.username username,"&chr(10)
Sql=Sql&"sum(c.Pv_2_Counter) Pv_2_Counter,"&chr(10)
Sql=Sql&"sum(c.Uv_2_Counter) Uv_2_Counter,"&chr(10)
Sql=Sql&"sum(c.Display_2_Counter) Display_2_Counter,"&chr(10)
Sql=Sql&"sum(c.ClickPv_2_Counter) ClickPv_2_Counter,"&chr(10)
...
Sql=Sql&"from CFWztg_AD_Counter_Day a inner join CFWztg_AD b on a.ad_id=b.id"&chr(10)
Sql=Sql&"where a.adddate >='"&StartDate&"' and a.adddate <='"&EndDate&"'"&chr(10)
Sql=Sql&"group by a.ad_id,a.adddate) c  inner join CFWztg_User d on c.adusername=d.username and d.upusername='"&CoUserName&"'"&chr(10)

If UserName<>"" Then Sql=Sql&" and d.username='"&UserName&"'"&chr(10)

Sql=Sql&"group by d.username,c.adddate order by c.adddate desc,AdUser_Spend_Cent desc"&chr(10)
...
<td><%=FormatNumber(Rs("CoUser_Income_CPA"),2,-1,0,0)%>(<%=FormatNumber(Rs("AdUser_Spend_CPA"),2,-1,0,0)%>)</td>
<td><%=FormatNumber(Rs("CoUser_Income_CPS"),2,-1,0,0)%>(<%=FormatNumber(Rs("AdUser_Spend_CPS"),2,-1,0,0)%>)</td>
<td><a href="?Action=daytjdetail&UserName=<%=Rs("UserName")%>&AddDate=<%=Rs("AddDate")%>" class="btn_add"><i class="mdi mdi-eye"></i> 详细</a></td>
