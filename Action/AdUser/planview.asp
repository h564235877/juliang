...
Sql = "Select a.*,b.planname From CFWztg_AD a inner join CFWztg_Plan b on a.Plan_ID=b.ID where a.AdState=2 And b.PlanState=2 And a.AdUserName='"&AdUserName&"' And a.Plan_ID=" & Plan_ID
...
<table class="tb_2">
<tr>
<td><span class="title">广告计划 <%=Rs("planname")%>[<%=Plan_ID%>] 下的正在运行的广告<a name="adnav" id="adnav"></a></span>
</td>

<td><a href="?Action=planlist" class="actionBtn"><i class="mdi mdi-keyboard-return"></i>返回列表</a>
</td>
</tr>
</table>
...
<a href="javascript:;" onclick="$(function () { confirm('确定要管理员暂停这个广告吗?', '', function (isConfirm) {if (isConfirm) {gotoUrl('../../AdUser.aspx?Action=adstop&ID=<%=Rs("ID")%>')}}, {confirmButtonText: '确定', cancelButtonText: '取消', width: 400});});" class="btn_del"><i class="mdi mdi-stop"></i> 暂停</a>
...
<a href="?Action=adeffectview&Ad_ID=<%=Rs("ID")%>" class="btn_add"><i class="mdi mdi-eye"></i>查看</a>
...
<a href="javascript:if(confirm('确定要暂停选择的这些广告吗?')){$('#form_adlist').attr('action','../../AdUser.aspx?Action=adselstop');$('#form_adlist').submit();}" class="btn btn-danger"><span>暂停选择的广告</span></a>
<a href="javascript:if(confirm('确定要删除选择的这些广告吗?')){$('#form_adlist').attr('action','../../AdUser.aspx?Action=adseldel');$('#form_adlist').submit();}" class="btn btn-danger"><span>删除选择的广告</span></a>
