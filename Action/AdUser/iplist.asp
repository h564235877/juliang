...
<%
Sql="Select AddDate From CFWztg_Visit_Ip Where AdUserName='"&AdUserName&"' Group By AddDate Order By AddDate Desc"
...
Call PxFilter(Px,"SpaceTime,UserName,Ip,Ad_ID,Ly,Ly_2,Keyword,AddTime,LastTime,AgentStr,Alexa,WebWidth,WebHeight,WebTop,WebLeft,MouseTop,MouseLeft,ScreenWidth,ScreenHeight,ScreenColorDepth,HisLen,WinOri,Hwc,Vendor,Renderer,Cct,Ostype,BrowserType,ErrorClickTotal,PvTotal,LastUrl,LastTime,FirstShowTime,VisitTotal,LowUnionUserName,Unit_Cent")
 ...
 If Ad_ID<>"" Then SqlW=" And Ad_ID="&Ad_ID
 If Ip<>"" Then SqlW=SqlW&" And Ip Like '%"&Ip&"%'"
 If IpArea_2<>"" Then SqlW=SqlW&" And IpArea_2="&IpArea_2
 If Ly<>"" Then SqlW=SqlW&" And Ly Like '%"&Ly&"%'"
 
Sql="Select Count(1) From CFWztg_Visit_Ip where AdUserName='"&AdUserName&"' And Datediff(d,AddDate,'"&AddDate&"')=0"&SqlW
Set Rs=Conn.execute(Sql)
IpTotal=Rs(0)


Sql="Select Top 2000 *,datediff(s,firstshowtime,addtime) SpaceTime From CFWztg_Visit_Ip WITH(NOLOCK) where AdUserName='"&AdUserName&"' And Datediff(d,AddDate,'"&AddDate&"')=0"&SqlW
...
<table class="tb_1">
  <tr class="tr_1"> 
	...
	<td><a href="<%=PageUrl%>&Px=SpaceTime">显点间隔</a></td>
